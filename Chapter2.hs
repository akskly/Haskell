...
-}
subList :: Int -> Int -> [a] -> [a]
subList a b lst  = if a < 0 || b < 0 then [] else drop (abs(a)) (take (abs(b) + 1) lst) 
...
takeEven n  = go 0 n
    where
      go :: Int -> [Int] -> [Int]
      go arr [] = []
      go arr (x:xs) = if mod arr 2 == 0 then x : go (arr + 1) xs else go (arr + 1) xs
...
smartReplicate l = concat (go l) 
    where
      go :: [Int] -> [[Int]]
      go []  = []
      go (x:xs) = concat (replicate x [x]) : go xs 
...
contains n l = go n l
    where
       go :: Int -> [[Int]] -> [[Int]]
       go _ [] = []
       go n (x:xs) = if elem n x then x : go n xs else go n xs 
...
Implement a function to rotaotherwisete a given finite list by N elements. Try
...
rotate a b 
        | a < 0 = []
        | b == [] = []
        | otherwise = drop a (take ((length b) + a ) (cycle b)) 
